...
- **API proxy:** `corolla.jhosan.top` is Cloudflare-proxied (used for the R2 custom domain too), and a Cloudflare Worker (`cloudflare/api-proxy-worker.js`) is routed at `corolla.jhosan.top/api/*` to forward those requests to the Render backend. This exists so the session cookie is first-party (its `Domain` defaults to whatever host the response appears to come from) instead of third-party — third-party cookies get blocked outright by Brave for every site and by Safari unless cross-site tracking protection is disabled, which was silently breaking sign-in persistence. `BACKEND_URL` (the GitHub secret baked into `app.js`) points at `https://corolla.jhosan.top` rather than the Render URL directly, so all frontend API calls go through the Worker. See the file's header comment for the Cloudflare dashboard setup steps (Workers & Pages → Create Worker → paste code → Deploy → Settings → Triggers → Routes → add `corolla.jhosan.top/api/*` on zone `jhosan.top`). No CORS handling is needed in the Worker itself — once traffic goes through this domain the browser treats it as same-origin.
...
Nine tables in SQLite via Turso (`@libsql/client` + `drizzle-orm/libsql`). Locally falls back to `file:./db.sqlite` when `TURSO_URL` is not set:

- **`products`** — `id, name, slug, phase, is_pack, created_at`. Phase 0 = tracked for pricing but not shown in the kit checklist. `is_pack` marks a product as a multi-buy/pack whose components should be added to Inventory when it's checked off in the Checklist — configured from the Inventory → Products sub-tab.
...
Product identity/management, split off the Prices tab so Prices can be a pure read-only price display. Renders into `#products-list` (`renderProductsPage()`, app.js). Reuses `liveProducts`/`allCategories`/`getCategoryAssignment()` the same way Prices does, and reuses most of the underlying product-CRUD functions Prices used to own directly (`saveProductName`, `deleteProduct`, `saveRetailerUrl`/`saveAddRetailerUrl`/`removeRetailer`, `saveNewProduct`, `changeProductCategory`) — those functions now call both `renderPricesTab()` and `renderProductsPage()` on success so both tabs stay in sync. **Shows no price anywhere.** Entirely gated on `syncEnabled` (both because its parent Inventory tab is already hidden when signed out, and as a defensive early-return inside `renderProductsPage()` itself).

Each product card has, in order: name (inline rename via the existing `toggleProductNameForm`/`saveProductName`), category `<select>` (`changeProductCategory`, unchanged), slug (inline edit — see cascading rename below), retailer URLs with edit/add/remove but no price (`toggleRetailerUrlForm`/`saveRetailerUrl`/`saveAddRetailerUrl`/`showRemoveRetailerConfirm`/`removeRetailer`, unchanged), an "Is Pack" toggle + component editor (see below), and a delete-product footer (`showDeleteProductConfirm`/`deleteProduct`, unchanged). A "+ Add product" form (`saveNewProduct`, unchanged) sits at the bottom.

**Pack configuration:** `packDrafts` (module-level, `{ [productId]: componentDraft[] }`) holds in-progress edits — nothing is sent to the backend until "Save pack" is clicked. Toggling "Is Pack" on lazily seeds a draft from `product.components` (or one blank row for a brand-new pack) and reveals the editor; toggling off an *already-saved* pack holds the checkbox visually checked and shows an inline confirm (`togglePackForCard`/`cancelUnpackConfirm`/`confirmUnpack`) before calling `DELETE /api/products/:id/pack`. Each component row (`packComponentRowHtml`) is either a reference to an existing product (`<select>` from `getAllProductSlugs()`, excluding the pack itself and any other pack — no recursive packs) or a free-text/inline item, with optional ml, an equipment checkbox, and — for inline rows only — a category/section picker that sets `sectionCategory`/`sectionLabel` (mirrors the old `sectionPath`). `addPackDraftRow`/`removePackDraftRow`/`updatePackDraftSlug`/`updatePackDraftField` mutate the draft in place; `savePackDraft` validates (≥1 component, every row needs a name), resolves each `slug` to a `componentProductId` via a fresh `liveProducts` lookup, and `PUT`s the whole list to `/api/products/:id/pack` — the backend replaces the pack's component rows atomically rather than diffing. On success (and after unpacking), `rebuildBundleComponents()` and `renderInventory()` run so the Inventory tab immediately reflects the change.
