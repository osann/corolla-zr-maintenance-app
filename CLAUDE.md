...
- **inventory** — product stock and spend tracker; four sub-tabs: "Inventory" (stock levels with progress bars, depletion forecast, drag-to-reorder categories), "Checklist" (kit purchase tracker, customisable phases, product prices), "Summary" (spend tracker, budget bar, live sale alerts), "Prices" (read-only price sheet with sparklines and sale badges, price alert thresholds)
...
All inventory data lives in `corolla-inventory-v1`. The Inventory tab has four sub-tabs: **Inventory** (stock levels), **Checklist** (kit purchase tracker), **Summary** (spend tracker + budget), **Prices** (price sheet + alerts). The old standalone Spend and Prices tabs no longer exist.

**Constants (`app.js`):**
- `INVENTORY_KEY` — storage key string `'corolla-inventory-v1'`
- `INV_CATEGORIES` — module-level array of 7 category objects, each with a `label` and `sections[]` (each section has a `label` and `slugs[]`). Same structure used by both `renderInventory()` (stock cards) and `renderPricesTab()` (price rows). Categories: Equipment, Pressure Washer Equipment, Exterior Wash, Exterior Protection, Interior Clean, Interior Protect, Wheels.
- `BUNDLE_COMPONENTS` — map of bundle slugs → component arrays `[{ name, ml?, equipment? }]`. Bundles are expanded to individual components in both inventory and depletion.
...
**State:** `let inventoryState = {}` — loaded from `INVENTORY_KEY`. Keys are either catalog slugs or composite keys `'{bundleSlug}:{componentName}'`. Each entry: `{ purchaseDate, volumeMl, remainingMl, manualOverride }`. `_order` is a special key holding the user's custom category sort order (string array of category labels).
...
- `initInventoryStock(slug)` — called when a kit item is checked in the Checklist. Initialises the inventory entry (or bundle component entries) with `volumeMl` from defaults and `remainingMl = volumeMl`. No-op if already initialised.
...
- `getInvCategoryOrder()` — returns ordered category labels: starts from `inventoryState._order` (user's saved order), adds any new defaults at the end, removes any that no longer exist in `INV_CATEGORIES`.
- `renderInventory()` — renders into `#inventory-list`. Iterates categories in `getInvCategoryOrder()` order. Each category is a draggable `.inv-category` with a header (`⠿` handle + label) and sections. Each section renders `.inv-card` items in a `.inv-grid`. Cards show product name, progress bar (colour-coded: green ≥50%, amber 20–50%, red <20%), remaining/total ml, sessions remaining estimate, and a volume adjustment control. Empty-state links point to the Checklist sub-tab.
...
**Drag-to-reorder categories:** `invCatDragSrc` (module-level, null by default) tracks the source category index. `dragstart`/`dragover`/`dragleave`/`dragend`/`drop` events are attached after each `renderInventory()` call. On drop, `inventoryState._order` is updated and `saveInventory()` + `renderInventory()` are called.

**Sub-tab navigation:** `.inv-sub-tab[data-inv-tab]` buttons show/hide `.inv-sub-panel` divs with matching `id="inv-sub-{tab}"`. Panel IDs: `inv-sub-stock` (Inventory), `inv-sub-checklist` (Checklist), `inv-sub-summary` (Summary), `inv-sub-prices` (Prices). The navigator is wired in `init()` via `document.querySelectorAll('.inv-sub-tab')`.
