...
//
// Uses real Playwright browser interaction first, then falls back to Bowden's Neto
// XHR. The previous plain-fetch Neto-only path received HTTP 403 from runner IPs.
...
async function main() {
  ...
  if (results.length === 0) {
    throw new Error('No Bowden\'s variant observations collected.');
  }
